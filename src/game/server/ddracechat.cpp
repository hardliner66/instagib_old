...
void CGameContext::ConPractice(IConsole::IResult *pResult, void *pUserData)
{
	...
	CGameTeams &Teams = ((CGameControllerCtf *)pSelf->m_pController)->m_Teams;
...
void CGameContext::ConLockTeam(IConsole::IResult *pResult, void *pUserData)
{
	...
	int Team = ((CGameControllerCtf *)pSelf->m_pController)->m_Teams.m_Core.Team(pResult->m_ClientID);

	bool Lock = ((CGameControllerCtf *)pSelf->m_pController)->m_Teams.TeamLocked(Team);
	...
	{
		((CGameControllerCtf *)pSelf->m_pController)->m_Teams.SetTeamLock(Team, true);
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
			if(((CGameControllerCtf *)pSelf->m_pController)->m_Teams.m_Core.Team(i) == Team)
				pSelf->SendChatTarget(i, aBuf);
	}
}
...
{
	((CGameControllerCtf *)m_pController)->m_Teams.SetTeamLock(Team, false);

	char aBuf[512];
	str_format(aBuf, sizeof(aBuf), "'%s' unlocked your team.", Server()->ClientName(ClientID));

	for(int i = 0; i < MAX_CLIENTS; i++)
		if(((CGameControllerCtf *)m_pController)->m_Teams.m_Core.Team(i) == Team)
	...
	else
	{
		...
		else
		{
			str_format(
				...
				sizeof(aBuf),
				"You are in team %d",
				((CGameControllerCtf *)pSelf->m_pController)->m_Teams.m_Core.Team(pResult->m_ClientID));
			pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "join",
				aBuf);
		}
	}
